/*
 * Read /proc/curproc/cmdline and /proc/curproc/file such that it can be tested
 * whether Valgrind intercepts the system calls that access these pseudo-files
 * properly and whether Valgrind does not modify the behavior of
...
int main(int argc, char** argv)
{
  ...
  test_cmdline(cwd, "/proc/curproc/cmdline", "/proc/curproc/cmdline");
  test_cmdline(cwd, "/proc/<pid>/cmdline", path);

  snprintf(path, sizeof(path), "/proc/%ld/file", (long) getpid());

  test_readlink(cwd, "/proc/curproc/file", "/proc/curproc/file");
  test_readlink(cwd, "/proc/<pid>/file", path);

  test_readlinkat(cwd, "/proc/curproc/file", "/proc/curproc/file");
  test_readlinkat(cwd, "/proc/<pid>/file", path);
